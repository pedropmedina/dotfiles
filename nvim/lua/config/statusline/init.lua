return function()
    ...
    local providers = {
        ...
        buffer_type = function()
            return gl_buffer.get_buffer_filetype() ~= '' and gl_buffer.get_buffer_filetype() ~= 'NVIMTREE' and gl_buffer.get_buffer_filetype() or ''
        end,
    } 
    ...
    local left = {
        ...
        utils.set_section('FileIcon', gl_fileinfo.get_file_icon, utils.buffer_not_empty, nil, colors.light_gray),
        utils.set_section('FileName', utils.root_path, utils.buffer_not_empty, nil, colors.light_gray),
        ...
        utils.set_section('DiagnosticHint', gl_diagnostic.get_diagnostic_hint, nil, '• ', colors.light_gray),
        utils.set_section('Separator_5'),
        utils.set_section('DiagnosticError', gl_diagnostic.get_diagnostic_error, nil, '• ', colors.red),
        utils.set_section('Separator_6'),
        utils.set_section('DiagnosticWarn', gl_diagnostic.get_diagnostic_warn, nil, '• ', colors.yellow)
    }

    local right = {
        utils.set_section('GitBranch', providers.git_branch, utils.has_git_branch),
        utils.set_section('DiffAdd', gl_vcs.diff_add, utils.has_git_branch, '+', colors.green, colors.gray),
        utils.set_section('DiffModified', gl_vcs.diff_modified, utils.has_git_branch, '~', colors.yellow, colors.gray),
        utils.set_section('DiffRemove', gl_vcs.diff_remove, utils.has_git_branch, '-', colors.red, colors.gray),
        ...
        utils.set_section('LineInfo', gl_fileinfo.line_column, nil, nil, colors.light_gray, colors.gray),
        utils.set_section('Separator_10', ' ', nil, nil, colors.light_gray, colors.gray),
        utils.set_section('PerCent', gl_fileinfo.current_line_percent, nil, nil, colors.light_gray, colors.gray),
    }

    local short_left = {
        utils.set_section('Separator_11', ' ', nil, nil, nil, colors.dark_black),
        utils.set_section('BufferType', providers.buffer_type, nil, nil, colors.gray, colors.dark_black)
    }
