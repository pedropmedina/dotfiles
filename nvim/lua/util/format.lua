...
-- Push lazy formatter to formatters list
...
-- Use conform plugin to handling formatting else fallback to LSP's formatexpr
...
-- Resolve formatters available for the given or current buffer
---@param buf? number
...
-- Notify user with information regarding resolved formatters for current buffer
---@param buf? number
function M.info(buf)
  buf = buf or vim.api.nvim_get_current_buf()
...
-- Check whether global or buffer autoformat is set for given buffer
---@param buf? number
function M.enabled(buf)
  ...
  -- Otherwise use the global value if available
  return gaf == nil or gaf
end

-- Toggle formatters on/off
---@param buf? boolean
...
-- Format current buf if formatter is present and enabled
